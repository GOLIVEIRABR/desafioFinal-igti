...
export default function modal(props) {
  const {isOpen, closeModalButton, update, selected} = props;
    ...
    >
      <div>
            <div className={css.flexRow+' '+css.centerH+' '+css.cendered+' '+css.spaceBetween}>
              <h5><strong>Edição de Lançamento</strong></h5>
            ...
            <form onSubmit={handleSubmit}>
              <div className={css.formModal}> 
                <div className={css.divRadioButtons}>
                  <label>
                    <input name="inputType" id="inputType" type="radio" defaultValue="-" defaultChecked={selected.type==='-'} />
                    <span className={css.modalSpanExpense}>Despesa</span>
                  </label>
                  <label>
                    <input name="inputType" id="inputType2" type="radio" defaultValue="+" defaultChecked={selected.type==='+'}/>
                    <span className={css.modalSpanEarning}>Receita</span>
                  </label>
                </div>
                <div className="input-field ">
                  <input onChange={handleInput} name="inputDescription" id="inputDescription" type="text" autoComplete="off" required defaultValue={selected.description}/>
                  <label htmlFor="inputDescription" className="active">Descrição:</label>
                </div>
                <div className="input-field ">
                  <input onChange={handleInput} name="inputCategory" id="inputCategory" type="text" autoComplete="off" required defaultValue={selected.category}/>
                  <label htmlFor="inputCategory" className="active">Categoria:</label>
                </div>                  
                
                <div className={css.flexRow+' '+css.centerH+' '+css.cendered+' '+css.spaceBetween}>
                  <div className="input-field" style={{marginLeft:'10px'}}>
                    <input name="inputValue" id="inputValue" type="number" min="0" step="0.01" required defaultValue={selected.value}/>
                    <label htmlFor="inputValue" className="active">Valor: </label>
                  </div>
                  <div className="input-field" style={{marginRight:'10px'}}>
                    <input name="inputDate" id="inputDate" placeholder="Data" type="date" className="browser-default"  required defaultValue={selected.yearMonthDay}/>
                  </div>
                </div>
              </div>
              <input id="buttonSubmit" type="submit" className="waves-effect waves-light btn" defaultValue="Salvar"></input>
            </form>
          </div>
    </ReactModal>
  )
